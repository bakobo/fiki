...
- O1, `@path` on KERIA. hio's WSGI server (`serving.py`) decodes the request path and re-encodes it with `quote`, so KERIA sees a re-encoding rather than the client's bytes; they differ for `~`, `%2F`, lowercase hex, and `: @ + = , ; ! $ & ' ( ) *`. Paths built from AIDs and SAIDs are unaffected, but identifier names chosen by users are not. The plan is for hio to expose the raw path as `RAW_URI` (scoped separately), for KERIA to use it when present, and until then to fall back to the re-encoded path, so such requests fail verification in canonical mode. The alternative, having Signify pre-normalize the path to hio's form, would be a KERI-specific rule and is rejected.
