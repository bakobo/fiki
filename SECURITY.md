...
All five language ports are ours, and a flaw in any one of them is in scope — including one that makes two ports disagree, since interoperating is the point. fiki deliberately has no KERI dependency; its only dependencies are a cryptography library and an RFC 8941 parser per language.

Signature verification is the whole product here, so treat anything that makes a bad signature verify, or a good one fail, as the most serious class of report.
